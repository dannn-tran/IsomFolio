...
The thumbnail **worker pool** generates a 640px JPEG per file into the catalog's thumbnail cache (640 covers the largest tile at 2× HiDPI without upscaling). The pool is a **fixed set of `Ncores` worker threads** pulling from a shared priority queue (a coordinator thread owns the queue for single-writer ordering/dedup and wakes workers via a condvar) — *not* a thread spawned per file. JPEGs are decoded at a **reduced DCT scale** (`jpeg-decoder`'s `scale()` picks the smallest 1/8·1/4·1/2 step still ≥ target), so a 24MP frame downscales several times faster, and with a fraction of the RAM, than a full decode; RAW uses the camera-embedded preview (`decode_raw_preview`), and anything unusual (CMYK/16-bit JPEG, png/webp/gif) falls back to `image::open`. The grid/list then render each tile with `image::Handle::from_path(cache_jpeg)` by default and hold **no decoded pixels** themselves — `iced_wgpu` decodes the JPEG on its own worker thread (never the UI thread, so fast fling doesn't stall — a not-yet-decoded tile just shows the loading placeholder for a frame or two) and keeps textures in a GPU atlas that it **trims** to the recently-drawn set. So baseline thumbnail RAM is `O(viewport)`, not `O(library)`. Decode is **reactive** (no predictive neighbour prefetch, unlike the loupe) — a hard fling shows the loading placeholder for a frame or two, same as Lightroom/Capture One. `ThumbnailState::Ready(path)` carries the cache path; generation status (Pending/Ready/Failed) is all the app tracks. (The loupe is separate — it owns decoded handles for the focused image + neighbour prefetch; see *Loupe image*.)

**Tier-2 decoded cache (`DecodedThumbCache`).** Atlas-trim has one sharp edge: opening the loupe/compare surface inserts a full-res image, which fires iced's trim and evicts *all* off-screen grid textures; returning to the grid then re-decodes every visible tile from disk on iced's single image worker — the "blank, then flow in one by one" return. To fix it the grid prefers a decoded RGBA `Handle` from `DecodedThumbCache` (`get`, falling back to `from_path` until warm); RGBA handles refill the atlas **synchronously**, so the round-trip is instant. *An app-side `from_rgba` handle map is allowed **only** when **byte-budgeted + LRU** (`THUMB_CACHE_BUDGET_BYTES`, ~128 MB).* The earlier version of this rule was an absolute ban — because the original handle cache was **unbounded**: a handle per file, decoded across *every* view, never evicted, so RAM grew with browse history (`O(library)`). The budget removes exactly that failure: residency is the visible window (`visible_file_ids`) plus a bounded LRU carry-over, evicted down to the cap, so steady-state RAM is `O(budget)` regardless of library size or how much you browse. Two seams keep it uniform across every thumbnail surface (grid, list, loupe filmstrip + fallback, compare) instead of per-site wiring: **lookup** is the single accessor `App::thumb_handle(id, path)` (get-or-`from_path`) every render site calls; **residency** is driven from one **chokepoint in `update`** — after each message `warm_thumbs_if_changed` re-warms only when the active surface's id set changes (gated by a hash). The warm policy lives in one place, `current_warm_ids` (match on `view_mode`: Browse/Preview→`visible_file_ids`, Loupe→±`LOUPE_FILMSTRIP_WINDOW` around the current frame, Compare→the compared set) — adding a surface is one arm, nothing else. The loupe *full-res* image stays a **separate** system (it decodes originals + prefetches neighbours; not folded in). Full rationale + framework survey in `dev-docs/thumbnail-cache.md`.
