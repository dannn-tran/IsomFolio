...
**Column count (X-axis)** is derived *analytically*, not measured from a scroll event. `cols()` computes the grid's usable width from `window_width` (tracked via window-resize events) minus the panels flanking the grid — sidebar, its resize handle, and the detail panel *once* when shown — then the grid's own `GRID_PADDING` and `GRID_SCROLLBAR_WIDTH`. The detail panel is a **sibling** of the grid in the layout row, so it already narrows the actual grid width: it must be charged **once**, never subtracted a second time (an earlier double-subtraction dropped a column whenever the panel opened). Deriving the count this way — rather than reading the scroll-sourced `viewport_width`, which is stale until the next scroll — keeps it exact the instant the window, sidebar, or detail panel changes width, with no scroll required. (`viewport_height` is still taken from the scroll event; height staleness only affects how many buffer rows are built, which is benign.)

### Folder tree

Built from the distinct indexed folder paths, reconstructing intermediate ancestors into a navigable tree. **Subfolders appear before indexing finishes:** a sync first does a cheap directory-only walk (`discover_dirs`, no file stat/decode) and hands the directories to the app via `FileEvent::FoldersDiscovered(Vec<(key, display)>)`; the app holds them in **`App::discovered_folders` (in-memory, session-only — not persisted)** and reloads the sidebar immediately, so acknowledging "add 8 subfolders" shows all 8 at once rather than after every image is indexed. `folder_tree(extra)` unions that in-memory set (count 0) with the files-derived `get_folder_counts`; `build_tree` merges by trie node, so a folder that later gains indexed files just accumulates its real count onto the same node (no duplication). **Why session-only, not a `folders` table:** the catalog is the source of truth *for files* and the watcher is a deliberate dirty-flag with no auto-reconcile — a persisted folder list would be the one entity with no orphan story, so a folder deleted on disk would become a permanent ghost row. Keeping it in memory means it never drifts from disk: it's seeded from a live walk on each add, pruned when a root is removed, and dropped on catalog switch; an *empty* folder simply doesn't survive a restart (appropriate for a photo catalog — a folder earns its place by containing photos, and once indexed it comes from the catalog anyway). **Anchored at the library roots**: the forest roots are the *anchors* — the deepest common ancestor of the added folders on each drive (`library_anchors`; a single anchor when they share a prefix, one per top-level segment when they span drives, possibly *virtual* if no added folder sits exactly there). Everything above the anchor (the noisy `/Users/me` prefix) is hidden, so breadcrumbs start at the user's content. Below an anchor, pure pass-through runs (folders with no own photos and exactly one child) are **compacted** into a single breadcrumb row — VS Code "compact folders" — so a deep single chain shows as `a / b / c` on one line, each segment separately clickable (`FolderNode.chain` holds the segments; `path`/`name` are the deepest). The intermediate names stay visible (not discarded). The leading separator of an absolute path is *not* a node — empty path segments are dropped at build time, so there is no nameless "ghost" root (the absolute prefix is re-attached when building top-level paths). With no known roots (unit tests) it falls back to filesystem-top roots. Counts shown are recursive (folder + all descendants). **Invariants:** expansion state is *ephemeral* (UI-only, not persisted); scan depth (recursive vs flat) is decided once when a root is added and *persisted per root*, and re-sync honours it. After a sync the freshly-added folder is revealed by seeding `expand_under_path` with the **normalised** path (it must match the case-folded node paths, dirty set and selection — the raw picker path will silently miss).
...
**Scroll-first prioritisation.** Within a view, the same mechanism follows the viewport: `Msg::Scrolled` calls `pool.prioritize(visible_file_ids())` so rows the user has just scrolled to generate ahead of the rest of that view's backlog (otherwise a long folder generates strictly top-to-bottom and scrolling jumps you past where the worker is). `App::visible_file_ids` mirrors the renderer's exact row window (including its `BUFFER_ROWS` over-scan), so what's prioritised is what's on (or just off) screen. It's gated twice to avoid hammering the coordinator: only when thumbnails are still `pending`, and only when the first visible row actually changed (`thumb_priority_row`) — not on every scroll pixel. A view load resets the gate so the next scroll re-evaluates.

**No high-res preview tier (deliberately).** The thumbnail is the *only* cached derivative. The loupe decodes the **original on demand** (`decode_image_for_display`) — so detailed inspection and 1:1 zoom always read true original pixels, never a downsized proxy. There is intentionally no eager 2048px "smart preview": measuring against the field (Lightroom/Capture One build such a preview, but use it *as* the main display image; Photo Mechanic/FastRawViewer build none and lean on embedded JPEGs), an eager high-res preview that the main view never reads is pure cost. It only ever served the offline loupe, and was removed to keep imports cheap and the feature set minimal. **Offline behaviour**: the grid still works offline (640px thumbnails are cached), and cull actions (flag/rate/colour/reject) are pure DB writes that apply when the drive returns — but the *loupe* of an offline photo can't decode (no original, no fallback), so detailed viewing needs the drive connected. **Cache hygiene**: `sweep_caches` runs on catalog open (off-thread), dropping thumbnails whose file id is no longer catalogued (folder removal, purge, external edits).

### Validation status (removable-drive / offline work)

Compile-verified on macOS (native) and Windows (cross-compile, incl. the `WM_DEVICECHANGE` path and `windows-sys` features). **Not yet exercised on real hardware**: an actual unplug/replug cycle (badge flips, cull persists, auto-clear on reconnect), a drive remounting under a new mount point (no duplicate import), and a real Windows run. The Linux `volume.rs` path is pure-std and reviewed but the crate couldn't be fully built locally (no cross C toolchain for bundled SQLite).
...
## Stacks (content-based grouping)

A **stack** groups near-identical frames shot seconds apart — bracketing, motor-drive bursts, "take five and pick one" — **inferred from pixels, not camera burst metadata**. It reuses the existing stack plumbing: the `files.burst_id` column is the group id, the `⧉ N` badge counts members, and the **⧉ Stack** toolbar toggle (`collapse_bursts`) collapses each group to one representative tile.

**Pipeline** (`isomfolio-core/src/phash.rs`):
- **dHash** — each file's cached 640px thumbnail is reduced to 9×8 grayscale; comparing horizontally-adjacent pixels yields a 64-bit difference hash. Perceptual distance = **Hamming distance** between hashes. Robust to resolution, mild compression, small tonal shifts.
- **Sharpness** — variance of the Laplacian over the thumbnail luma (a focus proxy). The **sharpest** frame becomes the collapse representative — `query_engine` orders by `phash_sharpness DESC` when collapsing, so the in-focus frame survives rather than merely the earliest.
- **Grouping** (`group_stacks`) — walk a folder's files in capture order; a frame joins the current stack when it is within `stack_threshold` Hamming distance of the stack's **first** frame (anchored — prevents slow drift chaining unrelated frames) **and** within `stack_window_secs` of the previous frame. Time is a **gate**, not the signal: same scene reshot an hour later starts a new stack; near-dupes across a 6 s gap (which the old ≤3 s time-only detector missed) still group. **Per-folder only** — identical frames in different folders never merge.

**Why a separate pass, not the thumbnail pipeline:** hashing needs the decoded thumbnail, which exists only after the thumbnail worker has run. So phash is its own opportunistic pass. `files_needing_phash` returns files with `phash IS NULL`; the app (`update/stacking.rs`) reads that list **under the catalog lock**, decodes + hashes the thumbnails **unlocked**, then writes `store_phashes` + `detect_and_store_stacks_all` **under the lock** — never decoding while holding the mutex (the same lock-discipline lesson as face clustering). Files whose thumbnail doesn't exist yet are skipped and re-tried on a later pass.

**Triggers:** `Msg::RunStacking` fires after a sync that adds files and whenever a thumbnail batch drains (`thumb_settled` hits zero) — so stacking improves incrementally as the user browses and thumbnails materialise. A `stacking_in_flight` guard coalesces overlapping triggers. Changing the threshold/window in Settings, or **Re-stack now**, re-runs grouping over the already-computed hashes (no re-hash needed — threshold only affects grouping). Settings: `auto_stack` (default on), `stack_threshold` (default 8), `stack_window_secs` (default 10).

**Edge case — unhashed files are never stacked:** a file with no `phash` yet is excluded from `detect_and_store_stacks` entirely (it can't be compared), so it shows as a singleton until its thumbnail is hashed, then joins on the next pass.
