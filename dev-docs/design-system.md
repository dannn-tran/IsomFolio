...
| Compare | — | Photo → Compare | `C` | N selected panes, synced zoom |
...
**Ordering & grouping — two stacked panels.** The sidebar is split into an **upper navigation panel** (scrollable, "where to look") and a **lower filtering panel** (pinned to the bottom, "how to narrow"). Top→bottom: catalog-name title · **search box** · `──` · *[scroll:]* **All Photos** · Folders · Albums · People · `──` · Imports · Deleted · *[pinned bottom:]* `──` · **Filters** · *(Open Catalog… footer)*. The **search box** is pinned above the scroll region (always reachable, never scrolls away), fenced by a `sidebar_divider()`. The **navigation panel** fills the middle and owns the whole scroll region (`FillPortion(2)`). The **Filters panel** is pinned at the bottom: its header (with `●` active marker) is always visible; collapsed by default — **revealed once on the first-ever launch** for discoverability, then collapsed on every later launch (one-time `seen_filters_hint` flag in `AppSettings`) — expanded it takes a bounded share (`FillPortion(1)`) and scrolls internally, so opening it *squeezes* — never *hides* — the navigation above.
...
| Grid tile (single) | Open in Loupe · Add to Album ▶ · Delete · — · Import XMP metadata · (Import Apple Finder tags, macOS) · Show in Finder / Locate… · Copy to Folder… |
...
**Reviewing similar shots — one surface, two layouts (`ViewMode::Compare`).** Culling near-duplicates is **manual** — there is no automated grouping (the phash/embedding clustering and the guided "Sift" pass were removed). The user sorts by capture time (bursts sit contiguous), range/Cmd-selects the candidates, and opens the **review surface** over that selection. It is **one surface with a layout switch** (`ReviewLayout { Survey, OneUp }`, top-bar `▦ Survey / ⛶ One-up`, `CompareSetLayout`) — *not* two features. (Rationale + competitor analysis: Lightroom E/C/N, Narrative/Aftershoot — every reference app presents one selection at multiple densities, one shared cursor.) Both layouts share **one scope** (`CompareState::files`), **one focus cursor** (`CompareState::focus`), and the same cull plumbing. `Space` flips the layout in place on the focused frame; `Esc` exits and restores the whole reviewed set as the grid selection (`exit_compare_to_grid`). **Entry always lands in Survey** (side by side — seeing the candidates together is the point): `C`, `Space` on a 2+ selection, and the single grid-context-menu **Compare** item all open Survey; One-up is a toggle away once inside, never a second door. (A single/empty selection `Space` opens the ordinary whole-view loupe.)

- **Survey layout** — N frames side by side, arranged either as a single horizontal **▭ Row** (default) or a roughly-square **▦ Grid** (`CompareState::survey_grid`, `CompareSetArrange`, a two-chip toggle in the top bar). **Both fit the window — no scroll:** Row shrinks the cells to fit one line, Grid wraps them into `ceil(√n)` columns. Every frame shares one `CompareState { zoom, pan }`, so the reused `LoupeImage` zooms/pans them in lockstep for a 100% comparison of the same region across candidates. **Modeless input:** a Survey click *focuses* the pane (`LoupeImage::on_click` → `CompareSetFocus`), **scroll** zooms and **drag** pans (both in lockstep); the cursor is a pointer, not a magnifier. (One-up leaves `on_click` unset, so a click toggles 1:1↔fit for pixel-peeking.)
- **One-up layout** — the focused frame big (shared zoom/pan) over a **filmstrip** of the set (click a thumb → `CompareSetFocus`; active ringed). `⊞ Lock zoom` (`CompareToggleZoomLock`) holds zoom across frames for a **blink comparison**; unlocked, stepping resets each frame to fit.
- **Shared across both:** one frame is *focused* (accent ring); `←`/`→` step the focus through the display order (clamped, no wrap, `step_focus`), or **click a Survey pane** / a One-up filmstrip thumb (`CompareSetFocus`); cull keys (`P`/`X`/`U`, ratings, labels) act on **the focused frame only** — `selection_target_ids` → `focused_file_id` in Compare, mirroring `loupe.idx` in the loupe. Pick/Reject badge updates live (`apply_flags_mem` patches the snapshot); `auto_advance_on_cull` steps focus to the next frame. **Sharpness ordering**: each scored frame shows `Sharp #k/total` (`sharpness_rank`), the clear winner gets the accented **◉ Sharpest** (`sharpest_slot`), and **`◉ Sharpest first`** (`CompareToggleSort`) reorders sharpest-first (`display_order`, unscored last).
- **Whittle-down (`R`, `CompareRemoveFocused`):** drops the focused frame from the review set — *not* a flag, just narrows the comparison to the finalists (the Lightroom-Survey ✕). Focus clamps to a neighbour; emptying the set exits to the grid. Compare-only (the key is global).

The shared **Browse layout switcher** (`▦ Grid / ▭ Strip / ⛶ Full`, `SetBrowseLayout`) puts the parallel `SurfaceLayout` vocabulary on the main toolbar, driving the Browse / Preview / Loupe modes. (The review surface uses its own `ReviewLayout`, deliberately separate from the Browse `SurfaceLayout`.)

**Sharpness is surfaced only relatively.** The computed focus score (variance-of-Laplacian) is **scene-dependent** — a low-detail scene scores low even in focus — so it is *never* shown as an absolute "blurry" verdict or an on-tile badge. It is used only where similar frames are compared directly: **Compare** marks the sharpest of its panes with a quiet **◉ Sharpest** chip in the caption strip (`CompareState::sharpest_slot`), computed **live** from each pane's decode (no stored column). The badge requires ≥2 scores and a clear margin (≥2%, so float noise / true ties aren't called a winner). Avoid reintroducing an absolute threshold cue — it was deliberately rejected as misleading.
