...
- **Survey layout** — N frames side by side. `grid_cols` defaults to a **single horizontal row** for a handful (`n ≤ 5` — the common "compare a few finalists" case), falling back to a roughly-square grid only when a row would squeeze (`n > 5`); a tooltip'd column control (`Columns · Auto 1 2 3 4`, `CompareSetCols`) overrides. Every frame shares one `CompareState { zoom, pan }`, so the reused `LoupeImage` zooms/pans them in lockstep for a 100% comparison of the same region across candidates.
