...
- **Compare** (`C`, 2+ selected) — N panes side by side, each fit to the window, with **synced zoom/pan**: every pane shares one `CompareState { zoom, pan }`, so the reused `LoupeImage` widget zooms/pans them in lockstep for a 100% comparison of the same region across candidates. The **sharpest** pane carries a quiet **◉ Sharpest** chip (`CompareState::sharpest_slot`, the old 2-frame rule generalised to N).
