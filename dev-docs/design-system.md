...
**Comparing similar shots (`ViewMode::Compare`).** Culling near-duplicates is **manual** — there is no automated grouping (the phash/embedding clustering and the guided "Sift" pass were removed). The user sorts by capture time (bursts sit contiguous), range/Cmd-selects the candidates, and reviews them on one of two existing surfaces, scoped to the selection. With 2+ tiles selected, **both surfaces appear in the grid context menu** ("Compare", "Review in Loupe") — the primary discovery path; the `C` / `Space` shortcuts and the View menu mirror them.
- **Compare** (`C`, 2+ selected) — N panes laid out in an **adaptive grid** (not a single squeezed row): `CompareState::grid_cols` defaults to roughly square (`ceil(√n)`), and a top-bar segmented control (`Grid · Auto 1 2 3`, `CompareSetCols`) pins the column count. Every pane shares one `CompareState { zoom, pan }`, so the reused `LoupeImage` widget zooms/pans them in lockstep for a 100% comparison of the same region across candidates. **Sharpness ordering** is surfaced two ways: each scored pane shows its place (`Sharp #k/total`, `CompareState::sharpness_rank`) and the clear winner gets the accented **◉ Sharpest** chip (`sharpest_slot`); a **`◉ Sharpest first`** toggle (`CompareToggleSort`) reorders the panes sharpest-first (`display_order`, unscored kept last) without disturbing the synced zoom. The column control chips carry tooltips (the bare `1 2 3` are otherwise opaque). **Compare is a full culling surface, not just a viewer:** one pane is *focused* (`CompareState::focus`, accent ring), `←`/`→` step the focus through the display order (clamped, no wrap, via `step_focus`), and the cull keys (`P`/`X`/`U`, ratings, labels) act on **that focused frame only** — `selection_target_ids` resolves to `focused_file_id` in Compare, just as it resolves to `loupe.idx` in the loupe. The pane's Pick/Reject badge updates live (`apply_flags_mem` patches the compare snapshot), and `auto_advance_on_cull` steps the focus to the next pane so a burst culls with repeated `P`/`X`.
- **Loupe scoped to the selection** (`Space`, 2+ selected) — `LoupeState::scope` holds the selected file indices; navigation steps only through them (filmstrip shows the selection, counter reads "Reviewing N selected"), with `lock_zoom` giving a blink comparison. A single/empty selection opens the normal whole-view loupe. On exit the scope clears but the **whole selection is restored to the grid** (scoped review lands back on its set, not collapsed to one frame); an unscoped loupe selects only the exit frame.
