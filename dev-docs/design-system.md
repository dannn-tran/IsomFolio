...
**Sift (`ViewMode::ResolveStacks`).** A guided pass that resolves *every* similar-shot group in the current view, one at a time — for when a shoot has many bursts to grind through. User-facing name is **Sift**; the two engines surface as **Sift · Bursts** (`R`, phash) and **Sift · Scenes** (`⇧R`, embedding), and a **`Sift (N)`** toolbar chip is the discoverable entry. It is a **view mode**, not a modal or docked panel: same surface class as Loupe and Compare (full content width, sidebar dropped, `Esc` retained, **no scrim**). Built on Compare's full-res decode path generalised to N frames; groups are gathered **uncollapsed**, capture-ordered, each pre-marking its **sharpest** frame as the default keeper. Key behaviours:
...
- **Live tolerance** (header slider): Scenes tune embedding `eps` ("Looseness"), Bursts tune the Hamming threshold ("Tolerance"). The view's clustering inputs are cached on entry (`SceneCache` / `BurstCache`) so the slider regroups **in memory** — no DB round-trip. Regroup fires on **release**, off-thread; a monotonic `regroup_seq` drops any superseded result so only the newest applies. Burst regroup is O(n) (instant); scene regroup is O(n²) DBSCAN, so a large scene set shows a frame-count **⚠** and a transient *Regrouping…*. Committed keep/reject survive (they're flags in the DB); only the unreached queue reshapes, and per-group decisions reset since the grouping changed.
