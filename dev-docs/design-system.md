...
**Grid tile image fit — fit, not crop.** Grid thumbnails use `ContentFit::Contain`: the **whole frame** shows, letterboxed in the square tile, with `BG_TILE_LOADING` filling the gaps so they read intentional. Culling decisions turn on composition and the frame *edges* a square crop (`Cover`) would hide, so fit is the convention for culling/catalog tools (Lightroom, Photo Mechanic, Bridge); square-crop is a social-publishing aesthetic, deliberately not used here. Loupe and the Sift survey grid already use Contain, so the whole app reads one frame consistently. Navigational **filmstrip** thumbnails (loupe strip, Sift Strip) keep `Cover` — they're tiny waypoints, not judgment surfaces. A future justified-mosaic grid (aspect-preserving variable tiles) could replace the letterbox, but fit-in-square is the correct default.
...
**Sift (`ViewMode::ResolveStacks`).** A guided pass that resolves *every* similar-shot group in the current view, one at a time — for when a shoot has many near-duplicates to grind through. User-facing name is **Sift**, a **single entry** (`R`, the **`Sift (N)`** toolbar chip, or **View → Sift…**). The two grouping engines (phash bursts, embedding scenes) are not separate entries — they're the two ends of one header slider (see *live tolerance* below); the title shows the active side (`Sift · Bursts` / `Sift · Scenes`). It is a **view mode**, not a modal or docked panel: same surface class as Loupe and Compare (full content width, sidebar dropped, `Esc` retained, **no scrim**). Built on Compare's full-res decode path generalised to N frames; groups are gathered **uncollapsed**, capture-ordered, each pre-marking its **sharpest** frame as the default keeper. Key behaviours:
- **Two layouts** (`SurfaceLayout`, header toggle `▦ Grid` / `▭ Strip`): *Grid* is the aspect-aware survey (columns from median frame aspect — 3 landscapes → a row of 3); *Strip* is a large one-up preview of the focused frame over a thumbnail filmstrip. Layout is **one stable choice for the whole pass** — set via the toggle, never re-decided per group (a per-group size-based auto was removed: it flipped Grid↔Strip between groups, which read as a bug). Keepers seed on entry from a saved in-session decision, else from the frames' existing **Pick** flags (so a keeper chosen in an earlier pass persists across exit/re-entry), else the sharpest.
- **Per-frame sharpness rank** on every frame (`★ sharpest` / `#2` / `#3`) so overriding the default keeper is informed — `StackReview` carries per-frame `sharpness`.
- **No Confirm step — decisions persist live.** Every keeper edit writes the group through to the catalog immediately (`commit_and_persist` → `persist_group_keepers`: keepers → Pick, rest → Reject), mirrors the result into the in-memory `StackReview` frame flags (so re-entry seeds from real flags), and records **one coalesced undo step per group** (`record_group_flags` merges consecutive same-frame-set edits). `Next ›`/`Enter` (`ResolveSkipStack`/`ResolveConfirm`) and `‹ Previous` only navigate — there is no apply message. A group **never touched** writes nothing (viewing isn't a decision), so re-opening Sift shows exactly what was decided, never a reset to auto-picks.
- **Keep/reject** by click (kept = `ACCENT` ring + "✓ Keep"; reject = dark scrim + "✕ Reject"), or keys: `P`/`X` keep/reject the **focused** frame (the Strip preview target; `SetFlag` is retargeted in Sift, not swallowed), `1`–`9` = toggle frame N by position, `0`/**↺ Apply auto-pick** = set the sharpest as keeper *and persist* (the explicit override of manual toggles). `Enter`/`Space` = next group. In Grid `←`/`→` step groups; in Strip they move frame focus and `Shift`+`←`/`→` steps groups. Keeping *nothing* writes a deliberate reject-all (undoable) — there's no Confirm guard because nothing is implicit.
- Grid selection is cleared on entry so stray flag/delete keys are inert. Exit (`Esc` or past-the-end) reloads the grid; all writes already landed, so there's no final-write race to chain.
- **Live tolerance** (one segmented header slider, normalized `ResolveState::tolerance` 0..1, seam at 0.5): the left half tunes the burst Hamming threshold (`0→16` bits), the right half the scene embedding `eps` (`0→1`); the active engine is derived from the position (`sift_is_scenes`). `Burst`/`Scene` end-labels light when active; the readout shows the effective `≤N bits` / `eps N.NN`. The burst inputs are cached on entry (`BurstCache` via `load_phashes`); the scene inputs (`SceneCache`, embeddings) are **lazy-loaded the first time the slider crosses the seam** (`SceneCacheLoaded`), so opening Sift never pays the embedding cost unless scenes are used. Regroups run **in memory**, on slider **release**, off-thread; a monotonic `regroup_seq` drops superseded results. Burst regroup is O(n) (instant); scene regroup is O(n²) DBSCAN, so a large scene set shows a frame-count **⚠** and a transient *Regrouping…*. Committed keep/reject survive (they're flags in the DB); only the unreached queue reshapes, and per-group decisions reset since the grouping changed.

The shared **Browse layout switcher** (`▦ Grid / ▭ Strip / ⛶ Full`, `SetBrowseLayout`) puts the same `SurfaceLayout` vocabulary on the main toolbar, driving the existing Browse / Preview / Loupe modes.

**Sharpness is surfaced only relatively.** The computed focus score (variance-of-Laplacian, `phash_sharpness`) is **scene-dependent** — a low-detail scene scores low even in focus — so it is *never* shown as an absolute "blurry" verdict or an on-tile badge. It is used only where two near-identical frames are compared directly: stacks pick the sharpest member as keeper, and **Compare** marks the sharper of its two frames with a quiet **◉ Sharper** chip in the caption strip (`CompareState::sharper_slot`). The badge requires both scores present and a clear margin (≥2%, so float noise / true ties aren't called a winner); missing scores (phash not yet computed) simply show no badge. Avoid reintroducing an absolute threshold cue — it was deliberately rejected as misleading.
...
| ⧉ (grid toolbar, icon toggle) | Collapse bursts (shots detected within ~3 s) to one representative tile (the sharpest). A burst tile carries a `⧉ N` badge (N = burst size); the badge also shows on burst members when not collapsed. Toggle off to cull within a burst. A *view* toggle — distinct from the **Sift (N)** button beside it, which launches the guided review. |
