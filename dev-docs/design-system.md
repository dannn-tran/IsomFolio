...
- **Per-group decisions persist** across `←`/`→` (`ResolveState::decisions`, write-through on every keeper edit) — stepping back never resets your marks.
- **Keep/reject** by click (kept = `ACCENT` ring + "✓ Keep"; reject = dark scrim + "✕ Reject"), or keys: `Enter`/`Space` = Keep & Next, `1`–`9` = toggle frame N, `0`/**↺ Reset to auto** = restore the sharpest pick. In Grid `←`/`→` step groups; in Strip they move frame focus and `Shift`+`←`/`→` steps groups. Keeping *nothing* is treated as Skip, never a silent reject-all.
- Grid selection is cleared on entry so stray flag/delete keys are inert; the final group's write chains to the exit reload (`ResolveFinished`) so the grid never shows a stale result.
