...
- **Two layouts** (`SurfaceLayout`, header toggle `▦ Grid` / `▭ Strip`): *Grid* is the aspect-aware survey (columns from median frame aspect — 3 landscapes → a row of 3); *Strip* is a large one-up preview of the focused frame over a thumbnail filmstrip. Groups > 4 frames auto-open Strip; a manual pick pins (`layout_pinned`).
