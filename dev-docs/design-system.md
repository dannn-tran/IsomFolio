...
**Reviewing similar shots — one surface, two layouts (`ViewMode::Compare`).** Culling near-duplicates is **manual** — there is no automated grouping (the phash/embedding clustering and the guided "Sift" pass were removed). The user sorts by capture time (bursts sit contiguous), range/Cmd-selects the candidates, and opens the **review surface** over that selection. It is **one surface with a layout switch** (`ReviewLayout { Survey, OneUp }`, top-bar `▦ Survey / ⛶ One-up`, `CompareSetLayout`) — *not* two features. (Rationale + competitor analysis: Lightroom E/C/N, Narrative/Aftershoot — every reference app presents one selection at multiple densities, one shared cursor.) Both layouts share **one scope** (`CompareState::files`), **one focus cursor** (`CompareState::focus`), and the same cull plumbing. `Space` flips the layout in place on the focused frame; `Esc` exits and restores the whole reviewed set as the grid selection (`exit_compare_to_grid`). Entry: `C` opens **Survey**, `Space` on a 2+ selection opens **One-up** (a single/empty selection `Space` opens the ordinary whole-view loupe); both also reachable from the grid context menu.

- **Survey layout** — N frames in an **adaptive grid** (not a squeezed row): `grid_cols` defaults to roughly square (`ceil(√n)`); a tooltip'd column control (`Columns · Auto 1 2 3`, `CompareSetCols`) pins it. Every frame shares one `CompareState { zoom, pan }`, so the reused `LoupeImage` zooms/pans them in lockstep for a 100% comparison of the same region across candidates.
