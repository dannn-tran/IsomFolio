...
- **Loupe scoped to the selection** (`Space`, 2+ selected) — `LoupeState::scope` holds the selected file indices; navigation steps only through them (filmstrip shows the selection, counter reads "Reviewing N selected"), with `lock_zoom` giving a blink comparison. A single/empty selection opens the normal whole-view loupe. Scope clears on exit.
