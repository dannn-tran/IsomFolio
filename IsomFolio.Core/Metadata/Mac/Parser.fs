module IsomFolio.Core.Metadata.Mac.Parser
...
module XAttrHelper =
    ...
    let private readXattrBytes attrName (filePath: string) : Result<byte[] option, string> =
    ...
    let getStringList attrName filePath =
        match filePath |> readXattrBytes attrName  with
        | Ok(Some bytes) -> parseBinaryPlist bytes |> Result.defaultValue []
        | _ -> []
...
[<SupportedOSPlatform("macos")>]
let getAppleMetadata (filePath: string): AppleMetadata =
    {
        UserTags = filePath
        |> XAttrHelper.getStringList Constants.UserTags
        |> List.map parseTag
    }